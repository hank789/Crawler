...
if (isset($_POST["action"]) && $_POST["action"] == "save_setup")
{
  ...
  if (!$rw_error_message) {
  	...
  	require_once '../../../classes/Urls.class.php';
  	$hosts=new Hosts();
  	$params=create_host_params_from_url($setuparray_combined['setup']['setURL']);
  	$hosts->host=$params['host'];
  	$hosts->port=$params['port'];
  	$hosts->https=$params['https'];
  	$urlMatch=array();
  	foreach ($setuparray_combined['setup']['addFollowMatch'] as $key=>$match){
  		if (!empty($match))
  			$urlMatch[]=$match;
  	}
  	$hosts->url_follow_rule=json_encode($urlMatch);
  	$hosts->link_post_date_rule=$setuparray_combined['setup']['setLinkPostDateRule'];
  	
  	//$is_host_exist=$hosts->find_by_host($params['host'],$params['https'],$params['port']);
  	$is_host_exist='';
  	if (!empty($is_host_exist)){
  		$hosts->Save($is_host_exist['id']);
  	}else{
  		$hosts->create();
  	}
  }
...
if (isset($_POST["action"]) && $_POST["action"]=="delete_setup")
{
  ...
  if (!$rw_error_message) {
  	$rw_message = "Setup was deleted.";
  	require_once '../../../classes/Utils.php';
  	require_once '../../../classes/Hosts.class.php';
  	require_once '../../../classes/Urls.class.php';
  	$hosts=new Hosts();
  	$params=create_host_params_from_url($val['setURL']);
  	$hosts->host=$params['host'];
  	$hosts->port=$params['port'];
  	$hosts->https=$params['https'];
  	$is_host_exist=$hosts->find_by_host($params['host'],$params['https'],$params['port']);
  	if (!empty($is_host_exist)){
  		$hosts->delete($is_host_exist['id']);
  	}
  }
}
...
<tr>
  <td colspan="3">
    ...
    <br><br>
    Post Date Match Rule &nbsp;<br>
    <input type="text" name="val[setLinkPostDateRule]" value="<?php if (isset($val["setLinkPostDateRule"])) echo $val["setLinkPostDateRule"]; ?>" size="50" style="width:400px">
    <br><br>
  </td>
</tr>
